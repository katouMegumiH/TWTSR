...
def cli_main():
    ...
    parser.add_argument("--seed", default=2025, type=int, help="random seeds. recommend aligning this argument with data split number to control randomness")
    parser.add_argument("--dataset_name", type=str, choices=["S1200", "ABCD", "UKB", "Dummy", "ADNI", "PPMI"], default="ADNI")
    parser.add_argument("--downstream_task", type=str, default="classification", help="downstream task")
    ...
    parser.add_argument("--classifier_module", default="ADNI", type=str, help="A name of lightning classifier module (outdated argument)")
    parser.add_argument("--loggername", default="tensorboard", type=str, help="A name of logger")
    parser.add_argument("--project_name", default="ADNI", type=str, help="A name of project (Neptune)")
    ...
    # parser.add_argument("--load_model_path", default="D:\Project\AD\SwiFT-main\project\output\PPMI\checkpt-epoch=17-valid_acc=1.00.ckpt", type=str, help="A path to the pre-trained model weight file (.pth)")
    parser.add_argument("--load_model_path", default=None, type=str, help="A path to the pre-trained model weight file (.pth)")

    parser.add_argument("--test_only", default=True, help="specify when you want to test the checkpoints (model weights)")
    parser.add_argument("--test_ckpt_path", type=str, help="D:\Project\AD\SwiFT-main\project\output\ADNI\checkpt-epoch=07-valid_acc=0.84.ckpt")
    ...
    ############################
    if args.test_only and args.test_ckpt_path:
        ckpt = torch.load(args.test_ckpt_path, map_location="cpu")
        sd = ckpt["state_dict"]

        drop = [k for k in sd if "attn._time_bias_table" in k]
        for k in drop:
            sd.pop(k)
        ...
        trainer.test(model, datamodule=data_module)  # 注意：这里不要再传 ckpt_path
        return
    ###############################
    # ------------ run -------------
    if args.test_only:
