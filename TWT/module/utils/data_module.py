...
from .data_preprocess_and_load.datasets import S1200, ABCD, UKB, Dummy, ADNIDataset, PPMI_ADNIDataset
...
class fMRIDataModule(pl.LightningDataModule):
    ...
    def get_dataset(self):
        ...
        #     return ADNIDataset
        elif self.hparams.dataset_name == 'PPMI' or self.hparams.dataset_name == 'ADNI':
            return PPMI_ADNIDataset
        else:
    ...
    def make_subject_dict(self):
        ...
        # elif self.hparams.dataset_name == "ADNI":
        #
        #     csv_path = os.path.join(self.hparams.image_path, "metadata", "sub_group.csv")
        #
        #     df = pd.read_csv(csv_path).dropna(subset=["SUB_ID", "DX_GROUP", "SEX"])
        #
        #     # 仅保留 0/1
        #
        #     df = df[df["DX_GROUP"].astype(int).isin([0, 1])].copy()
        #
        #     # 建索引（兼容 sub_001 / sub-001）
        #
        #     meta_by_id = {}
        #
        #     for _, row in df.iterrows():
        #         sid = str(row["SUB_ID"]).strip()
        #
        #         meta_by_id[sid] = row
        #
        #         alt = sid.replace("_", "-")
        #
        #         meta_by_id.setdefault(alt, row)
        #
        #     def lookup_row(folder_name: str):
        #
        #         cands = [folder_name, folder_name.replace("-", "_"), folder_name.replace("_", "-")]
        #
        #         if folder_name.startswith(("sub-", "sub_")):
        #             num = folder_name[4:]
        #
        #             cands += [f"sub_{num}", f"sub-{num}"]
        #
        #         for key in cands:
        #
        #             if key in meta_by_id:
        #                 return meta_by_id[key]
        #
        #         return None
        #
        #     subject_folders = sorted(os.listdir(img_root))
        #
        #     missing = []
        #
        #     for folder in subject_folders:
        #
        #         row = lookup_row(folder)
        #
        #         if row is None:
        ...
        #         sex = int(row["SEX"])
        #
        #         final_dict[folder] = [sex, target]
        #
        #     if missing:
        #         print(
        #             f"[ADNI] {len(missing)} subjects not found in CSV: {missing[:10]}{' ...' if len(missing) > 10 else ''}")
            ...
            # 2. 保留三分类 0 / 1 / 2
            df = df[df["DX_GROUP"].astype(int).isin([0, 1, 2])].copy()

            # 3. 建索引（兼容 sub_001 和 sub-001）
            meta_by_id = {}
            for _, row in df.iterrows():
                sid = str(row["SUB_ID"]).strip()
                meta_by_id[sid] = row
            ...
            for folder in subject_folders:
                ...
                sex = int(row["SEX"])  # 如果你要转0/1可在这里映射

                final_dict[folder] = [sex, target]

            if missing:
                print(
                    f"[PPMI/ADNI] {len(missing)} subjects not found in CSV: "
    ...
    def add_data_specific_args(cls, parent_parser: ArgumentParser, **kwargs) -> ArgumentParser:
        ...
        group.add_argument("--image_path", default='D:\\Project\\AD\\SwiFT-main\\project\\data\\ADNI\\fMRI', help="path to image datasets preprocessed for SwiFT")
